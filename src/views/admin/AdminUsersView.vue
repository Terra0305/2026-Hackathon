  ...
  <div class="max-w-[1240px] mx-auto px-4 sm:px-6 py-12 flex flex-col gap-10">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
      <div class="flex flex-col gap-2">
         ...
         <h1 class="text-3xl font-outfit font-bold tracking-tight text-sync-text flex items-center gap-3">
           사용자 관리
         </h1>
    ...
    <div class="grid grid-cols-1 overflow-hidden rounded-[2rem] border border-sync-border bg-black/5 dark:bg-white/5 backdrop-blur-md shadow-2xl">
       <div class="hidden md:grid grid-cols-12 gap-4 px-8 py-5 border-b border-sync-border bg-black/10 dark:bg-white/5">
         ...
         <div class="col-span-4 text-[11px] font-bold text-sync-muted uppercase tracking-widest text-center">Contact / Info</div>
         <div class="col-span-2 text-[11px] font-bold text-sync-muted uppercase tracking-widest text-center">Status</div>
         <div class="col-span-2 text-[11px] font-bold text-sync-muted uppercase tracking-widest text-right">Action</div>
       </div>

       <div v-for="user in filteredUsers" :key="user.id" class="grid grid-cols-1 md:grid-cols-12 gap-4 px-6 md:px-8 py-6 border-b border-sync-border last:border-0 items-center hover:bg-black/5 dark:hover:bg-white/5 transition-colors group">
         ...
         <div class="col-span-4 flex flex-col justify-center mt-2 md:mt-0 text-[11px] font-medium text-sync-muted gap-0.5 md:items-center">
            <span class="text-sync-text">{{ user.email || 'test@sync.com' }}</span>
            <span>{{ user.phone || '010-0000-0000' }}</span>
         </div>

         <div class="col-span-2 flex justify-center mt-4 md:mt-0">
            <span class="px-3 py-1 rounded w-max text-[10px] font-bold tracking-widest transition-colors"
              ...
              {{ user.status === 'up' ? '비휴면 (ACTIVE)' : '휴면 (INACTIVE)' }}
            </span>
         </div>

         <div class="col-span-2 flex items-center justify-end gap-2 mt-4 md:mt-0">
            <RouterLink :to="`/admin/users/${user.id}`" class="px-4 py-2 rounded-xl text-[11px] font-bold border border-sync-border text-sync-text hover:bg-sync-primary hover:text-white hover:border-sync-primary transition-all shadow-sm">
              관리하기
            </RouterLink>
         </div>
       </div>
